...
export async function GET(request: Request) {
  return NextResponse.json({ Welcome: "Welcome to square-root API" });
}

export async function POST(request: Request) {
  const data = await request.json();
  const { firstNumber, user: currentUser } = data;
  const operation = await getOperationCost({
    db,
    opType: OperationType.square_root,
  });
  ...
  if (Number(operation.cost) <= Number(currentUser.balance)) {
    ...
    const opResponse = Math.sqrt(Number(firstNumber))
